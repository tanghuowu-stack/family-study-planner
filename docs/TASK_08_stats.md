...
| 状态 | 判定 |
|---|---|
| `off` | 未来日；或休息日；或**非应做日**（起点前 / 排期未命中 / 该日 occurrence 被单独取消） |
| `done` | 应做日且已完成 |
| `missed` | 应做日且未完成（含今天——今天应做未完成显示 missed，但不断连续，见 §5） |
...
- 效果：休息日在所有项目月历显示为灰（off），连续计算时穿过不中断。
  注意：休息日优先于完成判定——即使当天做完了也按 off 处理，不计入连续。
