...
export default function App() {
  ...
  const changeStatus = async (task: TaskDisplay, status: TaskStatus) => { await repo().setDisplayStatus(task, status); refresh(); notify(status === "done" ? "已完成" : "状态已更新"); };
  const toggleChecklist = async (task: TaskDisplay, itemId: string) => { await repo().toggleChecklistItem(task.id, itemId, task.occurrenceDate); refresh(); };
  ...
  const actions = { onStatusChange: changeStatus, onChecklistToggle: toggleChecklist, onCopy: copyTask, onEdit: (task: Task) => setForm({ open: true, task }), onDelete: deleteTask, onOccurrenceCancel: cancelOccurrence, onOccurrencePostpone: postponeOccurrence };
  return <div className="min-h-screen bg-paper text-ink"><header className="screen-only sticky top-0 z-40 border-b border-stone-200/70 bg-paper/90 backdrop-blur-xl"><div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6"><button onClick={() => setPage("today")} className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-ink text-white"><CalendarCheck2 className="h-5 w-5" /></span><span className="text-left"><span className="block font-bold">小步计划</span><span className="hidden text-[10px] tracking-wider text-stone-400 sm:block">家庭学习生活规划</span></span></button><nav className="hidden items-center gap-1 lg:flex">{navItems.map(({ page: value, label, icon: Icon }) => <button key={value} onClick={() => setPage(value)} className={`flex items-center gap-1.5 rounded-xl px-3 py-2 text-sm font-medium ${page === value ? "bg-white shadow-sm" : "text-stone-500 hover:bg-white/60"}`}><Icon className="h-4 w-4" />{label}</button>)}</nav><div className="flex items-center gap-2">{!cloudInitializing && (<span className={`hidden items-center gap-1 text-xs sm:flex ${cloudMode ? "text-sage-600" : "text-stone-400"}`}><Cloud className="h-3.5 w-3.5" />{cloudMode ? "云端同步" : "本地模式"}</span>)}<button onClick={() => setForm({ open: true })} className="inline-flex items-center gap-2 rounded-xl bg-ink px-3.5 py-2.5 text-sm font-semibold text-white"><Plus className="h-4 w-4" /><span className="hidden sm:inline">添加任务</span></button></div></div></header>
    ...
    {page === "month" && <MonthPage date={selectedDate} refreshKey={refreshKey} onDateChange={setSelectedDate} onOpenDay={openDay} />}
    ...
    <nav className="screen-only fixed inset-x-2 bottom-2 z-40 grid grid-cols-5 rounded-2xl border border-stone-200 bg-white/95 p-1 shadow-card backdrop-blur lg:hidden">{navItems.map(({ page: value, label, icon: Icon }) => <button key={value} onClick={() => setPage(value)} className={`flex min-w-0 flex-col items-center gap-1 rounded-xl px-1 py-2 text-[10px] ${page === value ? "bg-sage-50 text-sage-700" : "text-stone-400"}`}><Icon className="h-5 w-5" /><span className="truncate">{label}</span></button>)}</nav>
    {form.open && <TaskForm task={form.task} initialDate={selectedDate} onClose={() => setForm({ open: false })} onSave={saveTask} />}
    {toast && <div className="fixed bottom-24 left-1/2 z-[70] -translate-x-1/2 rounded-full bg-ink px-5 py-2.5 text-sm text-white shadow-xl lg:bottom-8">{toast}</div>}
  </div>;
}
