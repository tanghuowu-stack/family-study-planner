...
- **今日页分组改为"上课 / 作业"两层**（只改今日页的「今日清单」「已完成」两区；任务管理页、逾期区、月历、周/月计划汇总不变）。规则全文见 `style.md` §2「今日页分组规则」。
  - **"上课"的口径**：需求写的是"contentType = 上课"，但代码里内容类型字段（`extraContentType`）**只有课外班任务才有**——游泳 / 钢琴 / 轮滑课（兴趣班）和校内任务根本没有这个字段。照字面判断，游泳课进不了上课组，校内数学作业会因为"内容类型为空"被兜底进"其他"而不是"数学"。所以改用已有的 `isCourseTask`，它就是任务行上那枚「上课」标签的判定，用户看到的"上课"和分组用的"上课"是同一个定义。"作业"= 其余全部，按原有 `taskSubjectGroup` 分学科（它自带"其他"兜底）；内容类型为空或遗留值的任务**按学科归类**，只有学科也判不出的才进"其他"——比需求示例里"统一归入其他"更细，但同样满足"不能消失"。
  - **排序机制的调整（这次的核心风险）**：`sortOrder` 是今日页与任务管理页共用的同一个字段，数据层的 `taskSort` 第一级按学科排、第二级才是 `sortOrder`。以前今日页按学科分组，学科级排序对组内没有影响；现在上课组混着各学科，若继续沿用 `taskSort` 的返回顺序，"把数学课拖到语文课上面"写完库会被学科默认序压回原位。改为今日页在 `groupDayTasks` 里按自己的桶重排（`dayBucketSort`：带时间置顶 → `sortOrder` → `createdAt`）。**`taskSort` 本身不改**——月历格子和逾期列表还在用它，改它会波及"只改今日页"的边界。用变异测试确认了这个风险是真的：去掉桶内重排后，端到端的跨学科拖拽用例正好失败。
  - **两页共用 `sortOrder` 的已知副作用（接受，不改模型）**：今日页上课组的一次拖拽会给不同学科、不同一级分类的课写 0..n-1；任务管理页按更细的二级类型分组，同一个二级类型里"课"和"作业"的 `sortOrder` 可能来自今日页两次不同的拖拽，相对顺序由最后一次拖拽决定、平局按创建时间。这和以前"今日页语文组合并了校内语文 + 课外语文"是同一类跨粒度现象，只是换了个切法。真要两边互不影响，得给今日页单独一个排序字段（涉及 migration + 四个映射面），这次不做。
  - **兜底加固**：学科顺序存在 `app_settings` 里，以前按存储值逐个渲染——存储值若缺某一科，那一科的任务整组不显示。新增 `normalizeSubjectOrder` 补齐缺失、去重、丢弃非法值；学科整组拖拽也改在规整后的序列上做（否则 `indexOf` 得 -1，`splice(-1)` 会误删最后一科）。
  - **测试** +18（`utils/__tests__/taskGrouping.test.ts`）：语数英课混排、游泳/钢琴/轮滑课进上课组、钢琴练习不算上课、作业按学科分、校内任务归学科、空内容类型 / 5 种遗留值 / 完全未知值的兜底、全品类混合下任务总数守恒无重复、存储学科序缺科仍显示、带时间置顶（含旧 `time` 字段）、上课组跨学科 `sortOrder` 生效，以及两条走真实 `reorderTasks` 写库的端到端拖拽。175 例全绿，`tsc -b` 通过。
  - **真实数据验证**：用 09-12 备份扫 7/1~9/12 共 74 天，每天分组后的任务数都等于数据层返回数，无一消失。真实日期 7/22 上课组 = FCE 精讲课（英语，带时间置顶）+ 奥数暑假班（数学）；8/3 = 大增暑假班（语文，带时间置顶）+ 钢琴课（兴趣班）；内容类型为空的「阅读并记录」进作业/其他。真实数据里没有"一天三科都有课"，也没有遗留内容类型值，这两类用 10 条临时任务在本地预览（本地模式、不联网）补测：三科课 + 游泳课混排、带时间的「10:10-12:30 奥数暑假班」置顶、空内容类型进数学、遗留「背诵」进英语、未知值进其他。拖拽用真实 DragEvent 驱动：上课组跨学科拖两次、作业/数学组内拖一次、学科卡片整组拖一次，每次都读库确认 `sortOrder` 写入，整页刷新后顺序保持。手机 375 / iPad 竖屏 768 / iPad 横屏 1024 三种尺寸无横向溢出，1024 下底栏正确隐藏。验证完重新导入备份，沙盒恢复原状（259 条、0 条临时任务、被拖过的真实任务 `sortOrder` 回到原值）。
  - 顺手订正 `style.md` 里过时的"页面底部内边距 `pb-28`"（08-18 已换成 `pb-content`）。
...
- **修复顺延任务"小项 5/5 但整体勾选框未完成"——这是 08-16 那次修复自己引入的回归**（用户反馈"大增语文课后作业"由 09-05 顺延而来、5 个小项全勾、进度 5/5、勾选框却是空的）。
  - **诊断过程中我先走了一步弯路，记下来供以后参考**：拿到反馈时旧备份已删，我按"大增系列是重复类任务"的印象做了合成复现，推出"occurrence 完成后 `findPendingOccurrenceDate` 跳到下一次欠账、而 checklist 全局共享"的机制。逻辑自洽、也确实能复现同样的表象——**但前提是错的**：拿到新备份一查，该任务是 `singleDate` + `autoNextDay`，0 条 occurrence，`recomputeStatusFromChecklist` 正常触发（日志有 `complete`）、库内 `status=done`、小项全 done，**底层数据完全正确**。教训：同名任务在库里有多种 `timeType`（大增系列既有 singleDate 也有 recurring 也有 dateRange），不核对真实字段就推断机制，再自洽也可能是在解另一道题。
  - **真实根因**：08-16 的 `rolledOverCompletedLate` 修复让顺延任务在"原定日到完成日之间"的每一天显示为 `status: "todo"`（这个判断本身是对的——那几天它确实还欠着）。但只覆盖了 `status`，没管 `checklistItems`——那是挂在任务本体上、**跨所有日期共享的同一份数组**（`TaskItem.tsx:188` 的进度条直接读它）。于是那几天勾选框读的是按日覆盖后的 todo、进度条读的是全局的 5/5，两个指示器时间维度不一致。修复前那几天任务是整条不出现的（上一个 bug），所以矛盾被藏着；修好"让它出现"之后才浮出来。用真实备份跑 `getTasksForDate` 逐日核对：09-05/08/11 全是"未完成 5/5"，09-12 才是"已完成 5/5"——用户截图同时带"由 09-05 顺延"徽章和未勾选状态，可锁定是翻在 09-06~09-11 之间。
  - **同一 bug 家族的共性**：完成状态有按天的维度（occurrence 表 / completedAt），checklist 没有；顺延机制专门把任务显示到"不属于它自己的那一天"，错位就暴露。上次是按天维度整个缺失（完成后消失），这次是给 status 补了按天维度但 checklist 没跟上。我上次诊断时把"checklist 无按天维度"定性为"已知设计限制、不是 bug"，判轻了。
  - **修复（展示层，不动数据模型）**：新增 `asStillOwed(task)`，把 status 覆盖为 todo 的同时把 checklist 全部覆盖为未勾选，三处按日覆盖点（dateRange 完成日前、singleDate 原定日、singleDate 顺延日）统一改用它；完成当天不走这条，显示真实勾选状态。checklist 没有按天维度、无法还原"那天勾了几项"，所以历史日统一显示为未开始——这是取舍，真要准确得给 `ChecklistItem` 加完成时间戳（涉及云端表结构和同步，另议）。
  - **顺带修了诊断时发现的另一个变体**：重复类任务 + 顺延，只有这一次排期、没有后续欠账时，完成后会从今日页**整条消失**（`findPendingOccurrenceDate` 只找未完成的、`scheduleOccursOn` 只认排期日，两条路都不命中）。在 occurrence 分支加第三条路：找 `completedAt` 落在今天、`occurrenceDate` 早于今天的已完成 occurrence，照实显示为已完成——与 singleDate 的 `rolledOverCompletedLate` 同一原则。真实数据里当前没有任务会踩到这条（唯一重复类带小项的任务是 skipIfMissed），属预防性修复。
  - 测试 +3（11g 顺延中间日 checklist 显示 0/5、完成当天真实 5/5、库内不受影响；11h dateRange 同一原则；11i 重复类单次排期完成后不消失）。**验证过用例有效性**：把修复暂存掉跑，正好这 3 条失败、其余 37 条通过。144 例全绿，tsc 通过。
  - **真实数据浏览器核实**：本地预览无登录会话，改为把用户导出的备份（259 条任务）通过 App 自己的"导入备份"路径灌入本地预览（本地模式、不联网）。在今日页从 09-12 逐日翻回 07-30，用「由 X 顺延」徽章锁定任务，读勾选框 aria-label 与进度文字。**7 个**受影响任务（周末作业 09-04→05、大增语文课后作业 09-05→12、数学周末作业 09-11→12、暑假班作业 08-20→30、奥数课后作业 07-30→08-07、暑假班作业 08-07→15、大增暑假作业 08-20→21；上一轮说 6 个是漏数了一个）全部：完成日 ☑ 且进度为真实值，中间日/原定日 ☐ 0/N。翻完后核对库内 7 条小项仍全 done，展示层未写回。
