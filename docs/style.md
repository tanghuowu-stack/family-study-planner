...
| `rose-100/400/500`（Tailwind） | — | 统计页打卡"漏卡"标记专用（月历漏卡日圆点 `bg-rose-100 text-rose-500`、图例 `bg-rose-400`），沿用危险操作同族色；与 `alert`（逾期）、`sun`（同步失败）语义互不替代（2026-07-18 补，同日随打卡重构删去已废弃的"单项格子红叉"用法）。 |
...
| 页面底部内边距（避让底栏）| `pb-content`（= `calc(6rem + env(safe-area-inset-bottom))`，见 `index.css`；原 `pb-28` 已于 2026-08-18 替换）|
...
| 分组标题内边距 | `px-3 py-2.5` |
| 分组间距 | `space-y-3` |
| 类型块间距（上课块 ↔ 作业块）| `space-y-5` |
| 类型标题与其下卡片间距 | `space-y-2` |

### 今日页分组规则（2026-09-25 起）

今日页的「今日清单」和「已完成」两区按**内容类型 → 学科**两层分组；逾期区不分组；**任务管理页不受影响**，仍按一级分类 → 二级类型分组。

```
上课                        ← 类型标题 h3（text-sm font-semibold text-ink, px-1）
┌──────────────────────┐
│ 所有上课任务混在一张卡片  │   ← 不按学科拆；无卡片标题；不可整组拖拽
└──────────────────────┘
作业                        ← 类型标题 h3
┌ 语文 ────────────────┐   ← 学科卡片，标题 h4（沿用分组标题样式）+ 拖拽手柄
┌ 数学 ────────────────┐   ← 学科卡片可整组拖拽调序，顺序持久化
┌ 英语 ────────────────┐
┌ 其他 ────────────────┐
```

- **"上课"的判定**：`isCourseTask`——与任务行上「上课」标签同一口径（课外班内容类型 = 上课，以及钢琴课 / 游泳课 / 轮滑课）。语数英课和兴趣班课都在这里；每行自带学科角标和「上课」标签，不按学科拆也能认出科目。
- **"作业"是其余全部**：课外班内容类型 = 作业、内容类型为空、历史遗留值（练习 / 听写 / 背诵 / 阅读 / 其他）、校内任务（本来就没有内容类型）、钢琴练习、临时事项。组内按 `taskSubjectGroup` 分语文 / 数学 / 英语 / 其他，"其他"是兜底——**每个任务恰好落进一个分组，不存在判断落空而消失的情况**。
- **排序**：每个卡片内，带具体时间的任务按时间置顶（不可拖），其余按拖拽顺序。上课卡片、各学科卡片内都可以逐条拖拽；学科卡片之间可以整组拖拽；「上课 / 作业」两块的先后固定。
- **空的块不渲染**：当天没课就没有「上课」标题，某学科没作业就没有那张卡片。
- 实现：`utils/taskGrouping.ts` 的 `groupDayTasks` / `dayBucketSort`（纯函数，有单测）；页面只负责渲染。
...
| 类型标题 | `text-sm font-semibold text-ink`（`px-1`）| 今日页「上课」「作业」两块的标题（复用"次要标题"的字号字重，不新增 token）|
