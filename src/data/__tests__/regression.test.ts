...
describe("R5 云同步不回滚新数据（LWW）", () => {
  ...
  it("9. remove 是软删除：写 deletedAt + 刷新 updatedAt，级联子任务，行不物理消失", async () => {
    ...
    const child = await taskRepository.create(baseDraft({ title: "子任务", parentTaskId: parent.id }));
...
describe("R5 allocateTask 重排", () => {
  it("10. 旧的未完成子任务软删不硬删，已完成子任务保留", async () => {
    const parent = await taskRepository.create(baseDraft({
      title: "作业周期",
...
describe("父子任务联动", () => {
  it("14. 子任务全部完成父任务自动 done，一个退回父任务回 todo", async () => {
    const parent = await taskRepository.create(baseDraft({ title: "父任务" }));
    const c1 = await taskRepository.create(baseDraft({ title: "子1", parentTaskId: parent.id }));
    const c2 = await taskRepository.create(baseDraft({ title: "子2", parentTaskId: parent.id }));
