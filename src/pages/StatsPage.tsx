...
import { isSoundEnabled, setSoundEnabled } from "../lib/completionSound";
...
export function StatsPage({ onImported, cloudMode }: { onImported: () => void; cloudMode?: boolean }) {
  ...
  return <><main className="screen-only mx-auto w-full max-w-4xl px-4 pb-28 pt-6 sm:px-6"><p className="text-xs font-semibold tracking-widest text-sage-700">STATS</p><h1 className="mt-1 text-2xl font-semibold">统计</h1>
    ...
    <section className="mt-6 rounded-2xl border border-stone-100 bg-white p-4 shadow-sm"><button onClick={() => setMoreSettingsOpen(!moreSettingsOpen)} className="flex w-full items-center justify-between text-left"><span className="font-semibold">更多设置</span><span className="inline-flex items-center gap-1 text-xs text-stone-500">{moreSettingsOpen ? "收起" : "展开"}<ChevronDown className={`h-4 w-4 transition ${moreSettingsOpen ? "rotate-180" : ""}`} /></span></button>
      {moreSettingsOpen && <div className="mt-4 space-y-4">
        <div><button onClick={() => setCloudSyncOpen(!cloudSyncOpen)} className="flex w-full items-center justify-between text-left"><span className="text-sm font-semibold">云同步</span><span className="inline-flex items-center gap-1 text-xs text-stone-500">{cloudSyncOpen ? "收起" : "展开"}<ChevronDown className={`h-4 w-4 transition ${cloudSyncOpen ? "rotate-180" : ""}`} /></span></button>{cloudSyncOpen && <div className="mt-3"><CloudLoginPanel cloudMode={cloudMode} /></div>}</div>
        ...
        <div className="border-t pt-4"><h3 className="text-sm font-semibold">完成音效</h3><div className="mt-2"><SettingCheck label="勾选完成时播放音效" checked={soundOn} onChange={(value) => { setSoundOn(value); setSoundEnabled(value); setMessage(value ? "完成音效已开启" : "完成音效已关闭"); }} /></div></div>
...
function CourseStatistics({ data }: { data: CourseStats }) {
  const groups = ["语文", "数学", "英语", "兴趣班", "其他"].map((group) => ({ group, items: data.items.filter((item) => item.group === group) })).filter((group) => group.items.length);
  return <div className="mt-3 rounded-xl bg-stone-50 p-3 text-sm"><p className="text-xs text-stone-400">{data.startDate} 至 {data.endDate}</p>{groups.length ? <div className="mt-2 grid gap-3 sm:grid-cols-2">{groups.map(({ group, items }) => <div key={group}><h4 className="text-xs font-semibold text-stone-600">{group}</h4>{items.map((item) => <div key={`${group}:${item.label}`} className="mt-1 flex justify-between gap-3 text-xs text-stone-600"><span className="truncate">{item.label}</span><span className="shrink-0 font-medium">{item.count} 节</span></div>)}</div>)}</div> : <p className="mt-2 text-xs text-stone-400">该日期范围内暂无课程</p>}<p className="mt-3 border-t pt-2 text-right font-semibold">合计：{data.total} 节</p></div>;
}
