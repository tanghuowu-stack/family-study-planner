  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink/30 backdrop-blur-sm sm:items-center sm:p-5" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><form onSubmit={submit} className="max-h-[95vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl bg-paper shadow-2xl sm:rounded-3xl">
    ...
    <div className="space-y-5 px-5 py-6 sm:px-7">
      ...
      {showMore && <div className="space-y-5 rounded-2xl border border-stone-100 bg-stone-50/50 px-4 py-5">
        ...
        <div className="grid gap-4 sm:grid-cols-3"><TimeField title="开始时间（可选）" value={draft.startTime} onChange={(startTime) => setDraft((current) => ({ ...current, startTime, endTime: startTime && current.estimatedMinutes ? addMinutesToTime(startTime, current.estimatedMinutes) : current.endTime }))} input={input} label={label} /><label className={label}>预计时长（分钟）<input type="number" min="1" step="1" inputMode="numeric" value={draft.estimatedMinutes ?? ""} onChange={(e) => { const raw = Number(e.target.value); const estimatedMinutes = Number.isInteger(raw) && raw > 0 ? raw : undefined; setDraft((current) => ({ ...current, estimatedMinutes, endTime: current.startTime && estimatedMinutes ? addMinutesToTime(current.startTime, estimatedMinutes) : current.endTime })); }} className={input} placeholder="任意正整数" /></label><TimeField title="结束时间（可选）" value={draft.endTime} onChange={(endTime) => set("endTime", endTime)} input={input} label={label} /></div>
...
// 原生 <input type="time"> 在部分移动端浏览器（尤其 iPad Safari）打开过选择器后不提供明确的"清空"方式，
// 显式加一个清除按钮，值非空时才显示（2026-07-20，同时是"清空时间保存后旧时间复活"bug 修复的配套 UI）
function TimeField({ title, value, onChange, input, label }: { title: string; value?: string; onChange: (value: string | undefined) => void; input: string; label: string }) { return <label className={label}>{title}<input type="time" value={value ?? ""} onChange={(event) => onChange(event.target.value || undefined)} className={input} />{value && <button type="button" onClick={() => onChange(undefined)} className="mt-1.5 w-full rounded-lg bg-stone-100 px-2 py-1.5 text-xs text-stone-600">清除，不设置时间</button>}</label>; }
