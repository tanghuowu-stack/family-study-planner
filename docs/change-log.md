...
- 修复本月计划统计：从 otherQuotas（weeklyQuota 路径）中排除课程类任务（extraHomework class / interestClass 上课），避免这类任务同时进入 interestGroups（按自然月逐日遍历）和 otherQuotas（按月周数计算）而导致重复计入或总数偏高。FCE精讲等课程现在只走 interestGroups，按自然月日期（2026-06-01 至 2026-06-30）实际 occurrence 统计。weeklyQuota 路径的 monthWeekCount 改回简洁的"本月有多少不同周起始日"计算，仅用于阅读/练习类 weekGoal 计划。确认任务管理 listAll() 不按日期过滤，「全部」可见所有未删除任务；「当前阶段（平时）」不显示暑假绑定任务是正确的阶段筛选行为。
