...
function timeLabel(task: Task) { if (task.timeType === "weekGoal") return `${task.weeklyQuota?.isWeeklyRecurring ? "每周执行｜" : ""}本周目标${task.weekStart ? `｜${fmtDate(task.weekStart)} 至 ${fmtDate(getWeekEndKey(task.weekStart))}` : ""}`; if (task.timeType === "assignmentWindow") return `作业周期：${fmtDate(task.assignmentWindow?.startDate)}～${fmtDate(task.assignmentWindow?.endDate)}`; if (task.schedulePattern === "specificDates") return `指定日期 ${formatSpecificDates(task.specificDates ?? [])}`; if (task.schedulePattern === "dailyRecurring") return `每日重复｜${fmtDate(task.recurrence?.startDate)} 至 ${task.recurrence?.endDate ? fmtDate(task.recurrence.endDate) : "长期"}`; if (task.schedulePattern === "dateRangeDaily") return `${fmtDate(task.startDate)}～${fmtDate(task.endDate)} 每天`; if (task.schedulePattern === "dateRangeWeekdays") return `${fmtDate(task.startDate)}～${fmtDate(task.endDate)}｜每周${weekdayText(task.rangeWeekdays)}`; if (task.schedulePattern === "weeklyRecurring" || task.timeType === "recurring") return `每周${weekdayText(task.recurrence?.weekdays)}｜${fmtDate(task.recurrence?.startDate)} 至 ${task.recurrence?.endDate ? fmtDate(task.recurrence.endDate) : "长期"}`; if (task.timeType === "dateRange") return `${fmtDate(task.startDate)}～${fmtDate(task.endDate)}`; return task.date ? fmtDate(task.date) : "未设置日期"; }
